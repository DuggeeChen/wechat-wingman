...
└───────────────┘                 │  侧栏裁剪 · JPEG 编码          │
  ...
  profile_ui.py（独立窗口）── 你粘贴聊天文本 ──► profile.py ──► profiles/<名字>.json
                                                       │
                                                       └── 背景块 ──► 上面那一轮纯文字请求
```
...
打开画像窗口，选一个联系人，把**你跟他的 50~100 条聊天记录以文本形式粘贴进去**（你自己从微信里复制出来；程序从不碰数据库）。然后：

- 它**在本地、确定性地**按说话人切分粘贴内容，所以「谁说的」这件事根本不需要模型猜。
- 模型提取观察，**每条都必须附带一句粘贴内容里的原话**。在文本里搜不到的引用会被丢掉，编造的「事实」进不来。
- 你逐条过目，不满意的可以拒绝，上一次导入可以一键撤销。
- 下次你读这个人的聊天时，画像会随纯文字那一轮一起带上，候选就是带着这些背景生成的。

**画像是你的，只留在本机。** 它存在 `profiles/`（已 gitignore），是你能直接读、直接删的纯 JSON，而且**永远不会发给其他联系人**。它只有两种情况会到模型那里：你主动建/更新画像时（粘贴内容原样全量发出），以及读**这个人**聊天时（只发 ≤600 字的摘要，不含原话）。具体发了什么见 [docs/PRIVACY.md](docs/PRIVACY.md)。
